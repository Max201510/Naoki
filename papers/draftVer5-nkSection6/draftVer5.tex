...
\naoki{In our application we are not solving this human error issues directly, but we are using the data mining and clustering methods to reduce the unrelated images. }
...
%Recently, by the increase of segmentation and the aerial image technology, the object-based change detection \textit{ Geographic Information System(GIS)} was developed. GIS was built based on the object-based change detection and it is mainly used for detecting the changes in geographical data by using the remote-sensing and the aerial images.
%Although, even the object-based change detection are able to detect the changes in wide area, in our paper, we used the pixel-based change detection. 
...
Clustering techniques were well studied and various clustering algorithms were developed. K-Means algorithm is the popular and oldest algorithm which is categorise in partitioning relocation methods. It was first used by James MacQueen. %\citep{macqueen1967}.
...
%Although, these segmentation may vary depends on the situations, and there is no algorithm which will fit to every situation. Therefore, the best objects segmentation algorithms are created for every situation.
%Even the object-based change detection is popular with the geographical detection, Pixel-based change detection is still use in other filed.

%The 3D GIS models were used for the building and geographical damage detection  before the SAR imagery was developed in 2003. Now, the SAR became a major damage detection algorithm.
%SAR is a remote sensing images created by using the radar echo. and it has a larger and meaningful object information can be taken in a image.

%Disadvantages, since the SAR is using the radar images the glasses and the small objects and the object condition can be strongly affect to the radar information. 
