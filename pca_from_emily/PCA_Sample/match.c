...
int Dist(Keypoint k1, Keypoint k2);
...
Keypoint CheckForMatch(Keypoint key, Keypoint klist)
{
    int dsq, distsq1 = 10000000000, distsq2 = 10000000000;
    ...
    */
	...
	if (distsq1 < SIFT_THRESH)
	  return minkey;
      }
    } else {
      /* check second nearest neighbor as well */
      if (PCAKEYS) { /* PCA-SIFT KEYS */
	if (distsq1 < PCA_MULT * distsq2)
	  return minkey;
      } else { /* SIFT KEYS */
